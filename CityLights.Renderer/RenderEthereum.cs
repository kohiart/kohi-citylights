...
internal sealed class RenderEthereum
{
    ...
    public static async Task<string> ExecuteAsync(string manifestPath, string folder, BigInteger token, int seed)
    {
        ...
        try
        {
            ...
            if (manifest == null || string.IsNullOrWhiteSpace(manifest.CityLightsParameters) ||
                string.IsNullOrWhiteSpace(manifest.CityLightsRenderer))
                throw new InvalidOperationException("Missing manifest!");

            var web3 = new Web3(account, rpc);
            var contractHandler = web3.Eth.GetContractHandler(manifest.CityLightsRenderer);
            var output = await RenderQueryAsync(contractHandler, token, seed, manifest.CityLightsParameters);
            ...
            filename = Path.Combine(folder, $"CityLights_Solidity_{token}_{seed}.png");
